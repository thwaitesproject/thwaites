...
def error(mesh_name, nx):
    ...
    u_source = 3.14159265358979*sin(3.14159265358979*x/L)/L + 1.0*x*sin(3.14159265358979*(-H2 + depth + y)/H2)**2/L**2 + x*cos(3.14159265358979*(-H2 + depth + y)/H2)**2/L**2 + 9.86960440108936*mu_v*x*cos(3.14159265358979*(-H2 + depth + y)/H2)/(H2**2*L) 
    v_source =  0.318309886183791*H2*sin(3.14159265358979*(-H2 + depth + y)/H2)*cos(3.14159265358979*(-H2 + depth + y)/H2)/L**2 - 3.14159265358979*sin(3.14159265358979*(-H2 + depth + y)/H2)/H2 - 3.14159265358979*mu_v*sin(3.14159265358979*(-H2 + depth + y)/H2)/(H2*L) 
    ...
    vel_outfile = File("vel_gz_mms_TSmelt_L"+str(L)+"_nx"+str(nx)+"justTS.pvd")
    vel_outfile.write(vel_, vel_ana_f)
    p_outfile = File("p_gz_mms_TSmelt_L"+str(L)+"_nx"+str(nx)+"justTS.pvd")
    p_outfile.write(p_, p_ana_f)
    temp_outfile = File("temp_gz_mms_TSmelt_L"+str(L)+"_nx"+str(nx)+"justTS.pvd")
    temp_outfile.write(temp, temp_ana_f)
    sal_outfile = File("sal_gz_mms_TSmelt_L"+str(L)+"_nx"+str(nx)+"justTS.pvd")
...
#    vp_timestepper.dt_const.assign(0.6*L/nx)
    ...
    while (sal_change>1e-6) or  (temp_change>1e-6): 
