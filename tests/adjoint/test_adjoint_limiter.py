...
def test_adjoint_limiter():
    ...
    limiter = VertexBasedP1DGLimiter(V)
    t = 0.0
    step = 0
    while t < T - 0.5*dt:
    ...
    print("sal max", q.dat.data_ro.max())
    print("sal min", q.dat.data_ro.min())

    print("J", J)
    tt = taylor_test(rf, q, h)
