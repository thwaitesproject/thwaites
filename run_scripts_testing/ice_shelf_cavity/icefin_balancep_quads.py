...
ice_draft = ice_gl + (y/L) * (ice_oo - ice_gl)
bathy  = bed_gl + (y/L) * (bed_oo - bed_gl)

ocean_thickness = ice_draft - bathy

Vc = mesh.coordinates.function_space()
# no mesh stretching
#f = Function(Vc).interpolate(as_vector([y, z*ocean_thickness + bathy]))
#mesh.coordinates.assign(f)
#y, z = SpatialCoordinate(mesh)

##### uncomment for roms surface squshing
##scale mesh to make ice shelf slope
f = Function(Vc).interpolate(as_vector([y, ocean_thickness*z - ocean_thickness]))
mesh.coordinates.assign(f)
y, z = SpatialCoordinate(mesh)
...
f = Function(Vc).interpolate(as_vector([y,depth_c*z_scaled + (ocean_thickness - depth_c)*Cs - -ice_draft ])) #+ (1.0 - offset_backward_step_approx(x,k,x0))*z]))
mesh.coordinates.assign(f)
y, z = SpatialCoordinate(mesh)
...
#hor = TensorProductElement(DG_1, CG_2)
#Ned_horiz = HCurlElement(hor)
#P2P1 = TensorProductElement(CG_2, DG_1)
...
# Define a dump file
dump_file = "/data/icefin_simulations/simple_forcing/08.11.22_thwaites_dt150.0_dtOutput864000.0_T8640000.0_isotropicdx5_iterative_600mdepth_ulim_kappa1e-4vert_muv1e-3_L100m_initTSconstant_umin0.01_nomelt_draguvtop/dump.h5"
...
#horizontal_source = conditional(y > (1.0-sponge_fraction) * L,
...
absorp_sal = conditional(y > (1.0-sponge_fraction) * L,
                         ...
                         0.0)
# Temperature source term
#source_u = conditional(y > (1.0-sponge_fraction) * L,
#                           absorption_factor * u_init *((y - (1.0-sponge_fraction) * L)/(L * sponge_fraction)),
#                          0.0)
#absorp_u = conditional(y > (1.0-sponge_fraction) * L,
#                         absorption_factor * ((y - (1.0-sponge_fraction) * L)/(L * sponge_fraction)),
#                         0.0)
source_u = absorption_factor * u_init
absorp_u = absorption_factor


# Interior penalty term
# 3*cot(min_angle)*(p+1)*p*nu_max/nu_min
#u_source = Constant(-f*u_init)

ip_alpha = Constant(3*dy/dz*2*ip_factor)
# Equation fields
vp_coupling = [{'pressure': 1}, {'velocity': 0}]
vp_fields = {'viscosity': mu, 'source': mom_source, 'coriolis_frequency': f, 'u_velocity': u, 'balance_pressure': p_b} #, 'interior_penalty': ip_alpha}
...
vp_bcs = {"top": {'un': no_normal_flow, 'drag': ice_drag}, 2: {}, #'stress': stress_open_boundary}, 
        1: {'un': no_normal_flow, 'drag':ice_drag}, "bottom": {'un': no_normal_flow, 'drag':ice_drag}} #, 'drag': ice_drag}}
...
frazil_bcs = {}
# dx = 2.5 m , use delta x = 1 m
melt_gradp = - g* (beta_sal * (dot(grad(sal), n) * 0.1 * - S_ref) - beta_temp * (dot(grad(temp), n) *0.1 * - T_ref))

p_b_bcs = {2:{'gradpb': -melt_gradp}}
