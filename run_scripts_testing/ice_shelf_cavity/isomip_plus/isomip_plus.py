# ISOMIP+ setup based off 3d wedge with coriolis
...
# create mesh
mesh = Mesh("./isomip_plus_3d_box.msh")
...
#mesh.coordinates.dat.data[:,2] = ((x/(0.6667*L))*(H2-H1) + H1)*z/H2

# shift z = 0 to surface of ocean. N.b z = 0 is outside domain.
PETSc.Sys.Print("Area of South side (Gl wall) should be 20*80e3 = 1.6e6m^2: ", assemble(Constant(1.0)*ds(103, domain=mesh)))

PETSc.Sys.Print("Area of North side (open ocean) should be 720*80e3=5.76e7m^2: ", assemble(Constant(1.0)*ds(104, domain=mesh)))

PETSc.Sys.Print("Area of bottom: should be 80e3*480e3 =3.84e10m^2: ", assemble(Constant(1.0)*ds(102, domain=mesh)))

PETSc.Sys.Print("Area of ocean surface should be 80e3*160e3 = 1.28e10", assemble(Constant(1.0)*ds(101, domain=mesh)))

#PETSc.Sys.Print("Area of East side should be 0.5(a+b)*h=5.2e5", assemble(Constant(1.0)*ds(108, domain=mesh)))

#PETSc.Sys.Print("Area of West side should also be 5.2e5", assemble(Constant(1.0)*ds(107, domain=mesh)))
...
vp_bcs = {106: {'un': no_normal_flow, 'drag': ice_drag}, 105: {'un': no_normal_flow}, 
        102: {'un': no_normal_flow, 'drag': 0.0025}, 101: {'un': no_normal_flow}, 
        104: {'un': no_normal_flow}, 103: {'un': no_normal_flow},
        107: {'un': no_normal_flow}, 108: {'un': no_normal_flow}}

temp_bcs = {106: {'flux': -mp.T_flux_bc}}

sal_bcs = {106: {'flux': -mp.S_flux_bc}}
...
# Set up folder
folder = "/data/3d_isomip_plus/first_tests/"+str(args.date)+"_3d_HJ99_gammafric_dt"+str(dt)+\
         ...
         +"_dxy2km_36layers_no_squashing_closed_iterative_initial_solve_coriolis/"
