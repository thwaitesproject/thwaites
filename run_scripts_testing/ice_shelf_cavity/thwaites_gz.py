...
# create mesh
mesh = Mesh("./2d_thwaites_grounding_zone_crevasse.msh")

PETSc.Sys.Print("Mesh dimension ", mesh.geometric_dimension())

# shift z = 0 to surface of ocean. N.b z = 0 is outside domain.
PETSc.Sys.Print("Length of lhs", assemble(Constant(1.0)*ds(2, domain=mesh)))

PETSc.Sys.Print("Length of rhs", assemble(Constant(1.0)*ds(1, domain=mesh)))
...
dump_file = "/data/2d_thwaites_gz/31.05.22_3_eq_param_ufricHJ99_dt60.0_dtOutput3600.0_T864000.0_isotropicdx5_open_2.5d_sponge_stablerestoring_limu_crevasse/dump_step_14400.h5"
...
sponge_fraction = 0.045  # fraction of domain where sponge (5 x 25m cells wide)
# Temperature source term
source_temp = conditional(y < sponge_fraction * L,
                          ((sponge_fraction * L - y)/(L * sponge_fraction)) * absorption_factor * T_restore,
                          0.0)

# Salinity source term
source_sal = conditional(y < sponge_fraction * L,
                         ((sponge_fraction * L - y)/(L * sponge_fraction)) * absorption_factor * S_restore,
                         0.0)

# Temperature absorption term
absorp_temp = conditional(y < sponge_fraction * L,
                          ((sponge_fraction * L - y)/(L * sponge_fraction)) * absorption_factor,
                          0.0)

# Salinity absorption term
absorp_sal = conditional(y < sponge_fraction * L,
                          ((sponge_fraction * L - y)/(L * sponge_fraction)) * absorption_factor,
                         0.0)
...
temp_fields = {'diffusivity': kappa_temp, 'velocity': v, 'source': source_temp, 'absorption coefficient': absorp_temp}
sal_fields = {'diffusivity': kappa_sal, 'velocity': v, 'source': source_sal, 'absorption coefficient': absorp_sal}

##########

# Get expressions used in melt rate parameterisation
mp = ThreeEqMeltRateParam(sal, temp, p, z, velocity=pow(dot(v, v) + pow(u, 2), 0.5), HJ99Gamma=True, ice_heat_flux=False)
...
vp_bcs = {4: {'un': no_normal_flow, 'drag': ice_drag}, 2: {'stress': stress_open_boundary}, 
        1: {'un': no_normal_flow}, 3: {'un': no_normal_flow, 'drag': 2.5e-3}}
u_bcs = {}
...
# Set up folder
folder = "/data/2d_thwaites_gz/"+str(args.date)+"_3_eq_param_ufricHJ99_dt"+str(dt)+\
         "_dtOutput"+str(output_dt)+"_T"+str(T)+"_isotropicdx5_open_2.5d_sponge_stablerestoring_limu_crevasse_from10daydump/"
