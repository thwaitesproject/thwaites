...
firedrake_folder_layered = "/data/2d_mitgcm_comparison/layered_meshes/01.06.20_3_eq_param_ufricHJ99_dt120.0_dtOutput3600.0_T4320000.0_ip50.0_tres86400.0_Kh0.001_openoceanKv0.001_dy50_layered_nz10_closed_iterative/"
firedrake_folder_layered_refined = "/data/2d_mitgcm_comparison/layered_meshes/01.06.20_3_eq_param_ufricHJ99_dt120.0_dtOutput3600.0_T4320000.0_ip50.0_tres86400.0_Kh0.001_openoceanKv0.001_dy50_layered_nz10_refined_top_bottom_dz0.2_closed_iterative/"
...
firedrake_df_layered = pd.read_hdf(firedrake_folder_layered+"matplotlib_arrays.h5", key="0")
firedrake_df_layered_refined = pd.read_hdf(firedrake_folder_layered_refined+"matplotlib_arrays.h5", key="0")

output_folder = "/data/mitgcm_comparison_videos/layered_meshes/2d_50days_refined_layered_01.06.20_3_eq_param_ufricHJ99_dt120.0_dtOutput3600.0_T4320000.0_ip50.0_tres86400.0_Kh0.001_openoceanKv0.001_dy50_nz10_closed_iterative/"
##########
...
# Get firedrake coordinates layered
y_array_fd_layered = firedrake_df_layered['y_array']
z_array_fd_layered = firedrake_df_layered['z_array']

# Get firedrake coordinates layered refined
y_array_fd_layered_refined = firedrake_df_layered_refined['y_array']
z_array_fd_layered_refined = firedrake_df_layered_refined['z_array']
...
def Three_comparison_plot(t, fields, same_colour_scale=True):
    fig = plt.figure(figsize=(20, 15))
    model = ["Constant dz = 1m, dt = 30s", "Layers = 10, dt = 120s", "Layers = 10, refined dz=0.2m, dt = 120s"]
    ax = []
    ...
    if same_colour_scale:
        cplotFD_z = ax[0].tricontourf(1e-3 * y_array_fd, z_array_fd, triangulation_fd, fields[0], fields[3], cmap='bwr',extend='both')
        cplotFD_layered = ax[1].tricontourf(1e-3 * y_array_fd_layered, z_array_fd_layered, fields[1], cplotFD_z.levels, cmap='bwr',extend='both')
        cplotFD_layered_refined = ax[2].tricontourf(1e-3 * y_array_fd_layered_refined, z_array_fd_layered_refined, fields[2], cplotFD_z.levels, cmap='bwr',extend='both')
    else:
        # probably dont want to use this as you need them on the same scale...
        cplotFD = ax[0].tricontourf(1e-3 * y_array_fd, z_array_fd, triangulation_fd, fields[0], 30, cmap='coolwarm')
        cplotMIT = ax[1].tricontourf(1e-3 * y_array_fd_layered, z_array_fd_layered, fields[1], 30, cmap='PRGn')
    
    # Add colour bar
    cbar0 = plt.colorbar(cplotFD_z, ax=ax[0], shrink=1.0)
    cbar0.set_label(fields[4], fontsize=22)
    cbar1 = plt.colorbar(cplotFD_layered, ax=ax[1], shrink=1.0)
    cbar1.set_label(fields[4], fontsize=22)
    cbar2 = plt.colorbar(cplotFD_layered_refined, ax=ax[2], shrink=1.0)
    cbar1.set_label(fields[4], fontsize=22) 
    
    # Save and close figure
    if t < 10:
        plt.savefig(output_folder + fields[5] + "0" + str(t) + "hours.png")
    else:
        plt.savefig(output_folder + fields[5] + str(t) + "hours.png")
    
    plt.close()
    return cplotFD_z.levels 
...
for t in range(1,T_end_hours+1):
    ...
    # Read in firedrake arrays layers
    v_fd_layered = firedrake_df_layered['v_array_'+str(t)+'hours']
    w_fd_layered = firedrake_df_layered['w_array_'+str(t)+'hours']
    vel_mag_fd_layered = firedrake_df_layered['vel_mag_array_'+str(t)+'hours']
    temp_fd_layered = firedrake_df_layered['temp_array_'+str(t)+'hours']
    sal_fd_layered = firedrake_df_layered['sal_array_'+str(t)+'hours']
    rho_fd_layered = firedrake_df_layered['rho_array_'+str(t)+'hours']

    # Read in firedrake arrays layers refined
    v_fd_layered_refined = firedrake_df_layered_refined['v_array_'+str(t)+'hours']
    w_fd_layered_refined = firedrake_df_layered_refined['w_array_'+str(t)+'hours']
    vel_mag_fd_layered_refined = firedrake_df_layered_refined['vel_mag_array_'+str(t)+'hours']
    temp_fd_layered_refined = firedrake_df_layered_refined['temp_array_'+str(t)+'hours']
    sal_fd_layered_refined = firedrake_df_layered_refined['sal_array_'+str(t)+'hours']
    rho_fd_layered_refined = firedrake_df_layered_refined['rho_array_'+str(t)+'hours']
    ...
    temperature_plot = [temp_fd, temp_fd_layered, temp_fd_layered_refined, temp_levels, "Temperature / $^{\circ}$C", "temperature_"]
    salinity_plot = [sal_fd, sal_fd_layered, sal_fd_layered_refined, sal_levels, "Salinity / PSU", "salinity_"]
    v_plot = [v_fd, v_fd_layered, v_fd_layered_refined, v_levels, "v (meridional) / m/s", "v_velocity_"]
    w_plot = [w_fd, w_fd_layered, w_fd_layered_refined, w_levels, "w (vertical) / m/s", "w_velocity_"]
