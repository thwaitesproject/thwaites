...
# create mesh
mesh = Mesh("./2d_layered_ice_shelf_refined_top_bottom_dz0.2_nz10_dx50.msh")
...
open_ocean_kappa_v = Constant(args.Kh)
grounding_line_kappa_v = Constant(open_ocean_kappa_v/50.)
...
# Set up folder
folder = "/data/2d_mitgcm_comparison/layered_meshes/"+str(args.date)+"_3_eq_param_ufricHJ99_dt"+str(dt)+\
         "_dtOutput"+str(output_dt)+"_T"+str(T)+"_ip"+str(ip_factor.values()[0])+\
         "_tres"+str(restoring_time)+"_Kh"+str(kappa_h.values()[0])+"_openoceanKv"+str(open_ocean_kappa_v.values()[0])\
         +"_dy50_layered_nz10_refined_top_bottom_dz0.2_closed_iterative/"
