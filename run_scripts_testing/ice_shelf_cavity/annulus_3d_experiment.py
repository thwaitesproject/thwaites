...
H = 14.
water_depth = H
nR = 10  # number of cells in the radial direction
nT = 20  # number of cells in the azimuthal direction
nZ = 5  # number of cells in the vertical direction
dz = H/nZ #40.0
iR = 2.5  # inner radius (cm)
oR = 8  # outer radius (cm)

 
base_mesh = Mesh("./annulus_surface_nR10_nT20.msh") 
layers = []
cell = 0
yr = 0
...
f = Function(Vc).interpolate(as_vector([x, y, z-water_depth]))
...
mesh_refine = Function(P0_extruded).assign(0)
mesh_r_file = File("test_3d_annulus.pvd")
...
PETSc.Sys.Print("Area of inside  should be {:.0f}m^2: ".format(2*pi*iR*H), assemble((Constant(1.0)*ds(1, domain=mesh))))

PETSc.Sys.Print("Area of outside should be {:.0f}m^2: ".format(2*pi*oR*H), assemble(Constant(1.0)*ds(2, domain=mesh)))

PETSc.Sys.Print("Area of bottom: should be {:.0f}m^2: ".format(pi*(oR**2 -iR**2) ), assemble(Constant(1.0)*ds("bottom", domain=mesh)))

PETSc.Sys.Print("Area of ocean surface should be {:.0f}m^2".format(pi*(oR**2 -iR**2)), assemble(Constant(1.0)*ds("top", domain=mesh)))
...
if DUMP:
    ...
    T_inner = 18.  # Temperature of inner wall (degC)
    T_outer = 22.  # Temperature of outer wall (degC)
    T_surface = T_outer
    T_bottom = T_inner
...
T_ref = Constant(T_inner) 
beta_temp = Constant(3.3E-4)
g = Constant(981)  #  gravitational acceleration in cm /s ^2 !
mom_source = as_vector((0.,0.,-g))*(-beta_temp*(temp - T_ref) )

rho0 = 1050   # 1043 - 1053 kg / m^3 in Read 2003
rho.interpolate(rho0*(1.0-beta_temp * (temp - T_ref))) 
rho_anomaly_projector = Projector(-beta_temp * (temp - T_ref) , rho_anomaly)
...
# given logic above maybe sin() -> 1 so f = 2 omega. omega = 1 rad/s
f = Constant(2)
...
# set Viscosity/diffusivity (cm^2/s)
mu_h = Constant(2.1e-2)  # viscosity cm^2/s but apparently 1.8e-2 cm^2/s might have been more accurate 
kappa_h = Constant(1.3e-3)  # viscosity cm^2/s but apparently 1.8e-2 cm^2/s might have been more accurate 
mu_v = mu_h
kappa_v = kappa_h
...
no_normal_flow = 0.
no_normal_no_slip = as_vector([0,0,0])
...
vp_bcs = {"top": {'u': no_normal_no_slip}, 
        1: {'u': no_normal_no_slip}, 2: {'u': no_normal_no_slip}, 
        "bottom": {'u': no_normal_no_slip}} 

temp_bcs = {1: {'q': T_inner}, 2: {'q':T_outer}}
...
# Set up Vectorfolder
folder = "/data/ice_ocean/rotating_tank/"+str(args.date)+"_3d_rotatingtank_MaddisonRead_dt"+str(dt)+\
         "_dtOut"+str(output_dt)+"_T"+str(T)+"_Mu"+str(mu_h.values()[0])+"_K"+str(kappa_h.values()[0])+"_nT"+str(nT)+"_nR"+str(nR)+"_lay"+str(nZ)+"_closed_coriolis_fromrest/" #_smagviscmax"+str(max_nu)+"_officeshelf/"
