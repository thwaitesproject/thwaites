...
if not DUMP:
   # mesh = Mesh(f"./3d_crevasse_flume_dx250mto20m_dz5m_crevdxz5m_{args.angle}.msh", name=f"3dcrevasse_{args.angle}")
    mesh = BoxMesh(10, 10, 10, 5e3, 5e3, 100)#Mesh(f"./3d_crevasse_flume_dx250mto20m_dz2to5to10m_crevdxz5m_along.msh", name=f"3dcrevasse_{args.angle}")
...
PETSc.Sys.Print("Length of bottom", assemble(Constant(1.0)*ds(5, domain=mesh)))

PETSc.Sys.Print("Length of top", assemble(Constant(1.0)*ds(6, domain=mesh)))
...
if DUMP:
    ...
    v_init = as_vector((-0.01, 0, 0))#zero(mesh.geometric_dimension())
    ...
    frazil_init = Constant(0) #5e-9) # initialise with a minimum frazil ice concentration
    frazil.assign(frazil_init)
...
kappa = as_tensor([[1, 0, 0], [0, 1, 0], [0, 0, 1e-3]])
...
vp_fields = {'viscosity': mu, 'source': mom_source, 'coriolis_frequency': f, 'balance_pressure': p_b} #, 'interior_penalty': ip_alpha}
...
p_b_fields = {'buoyancy': mom_source, 'coriolis_frequency': f, 'velocity': v}
...
#p_b.interpolate(-g*(Temperature_term + Salinity_term)-horizontal_stress*y)
p_b.interpolate(-horizontal_stress*y)
...
temp_bcs = {3: {'qadv': T_restore}, 4: {'qadv': T_restore}, 1: {'qadv': T_restore}, 2: {'qadv': T_restore}}
...
sal_bcs = {3:{'qadv': S_restore}, 1:{'qadv': S_restore}, 4:{'qadv': S_restore}, 2:{'qadv': S_restore}}
...
strong_bcs = [] #DirichletBC(M.sub(0).sub(2), 0, (5,6)), 
                #DirichletBC(M.sub(1), y*horizontal_stress, (1,2)),
                #DirichletBC(M.sub(1), 0, 3),
                #DirichletBC(M.sub(1), 5000*horizontal_stress, 4)]
##########
...
# Set up folder
folder = f"/data0/wis15/3d_crevasse/{args.date}_5kmflume_3_eq_param_nomelt_dt{dt}_dtOutput{output_dt}_T{T}_3d_openbox_bodyforce_posv_qice0_dx500m_dz10_qadv_balp_nofriction_mu1e-3_advectionds/"
