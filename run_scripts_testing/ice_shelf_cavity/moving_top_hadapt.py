...
def run_simulation(mesh, t_start, t_end, sal0, temp0, v0, p0):
    ...
    # Project values from previous mesh
    interpolate(v0, v_)
    interpolate(p0, p_)
    interpolate(sal0, sal)
    interpolate(temp0, temp)
    ...
    ramp = conditional(y > 1e3, conditional(y<4e3, 1, 0), 0) # Prevent high melting at inflow/outflow from distorting the mesh
    L = dot(phi,  ramp*1e4*mp.wb * n) * ds(4)
    ...
    folder = "/mnt/c/Users/wills/Documents/ice_ocean/moving_mesh/"+str(args.date)+"_3_eq_param_ufricHJ99_dt"+str(dt)+\
             "_dtOutput"+str(output_dt)+"_T"+str(T)+"_crevasse_moving_alecorr_nolim_hadapt_noendmelt_amp1e4/"
    ...
    v_file = VTKFile(folder+f"mesh{counter}/vw_velocity.pvd")
    v_file.write(v_)

    p_file = VTKFile(folder+f"mesh{counter}/pressure.pvd")
    p_file.write(p_)


    t_file = VTKFile(folder+f"mesh{counter}/temperature.pvd")
    t_file.write(temp)

    s_file = VTKFile(folder+f"mesh{counter}/salinity.pvd")
    s_file.write(sal)

    rho_file = VTKFile(folder+f"mesh{counter}/density.pvd")
    rho_file.write(rho)

    ##########

    m_file = VTKFile(folder+f"mesh{counter}/melt.pvd")
    m_file.write(melt)
    melt_top_file = VTKFile(folder+f"mesh{counter}/melt_top.pvd")
    ...
    return mover.mesh, sal, temp, v_, p_


num_adaptations = 40
interval_length = T / num_adaptations
...
metric_params = {
    "dm_plex_metric": {
        "target_complexity": 5000.0,
        "p": 2.0,  # normalisation order
        "h_min": 5,  # minimum allowed edge length
        "h_max": 50,  # maximum allowed edge length
    }
}
...
for i in range(num_adaptations):
    ...
    mesh, sal, temp, v_, p_ = run_simulation(mesh, t0, t1, sal, temp, v_func, p_func)
    v_func = Function(v_).assign(v_)
    p_func = Function(p_).assign(p_)
    
    # Adapt the mesh based on the concentration field at t
    P1 = FunctionSpace(mesh, 'CG', 1)
    sal_P1 = Function(P1).interpolate(sal)
    mesh = adapt_classical(mesh, sal_P1)
