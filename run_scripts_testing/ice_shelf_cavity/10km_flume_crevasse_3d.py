...
def construct_metric(mesh, **kwargs):
 #   P1 = FunctionSpace(mesh, "CG", 1)
 #   f = Function(P1).project(sal)
 #   H = recover_hessian(f)
 #   M = hessian_metric(H)
    P1_ten = TensorFunctionSpace(mesh, "CG", 1)
    M = Function(P1_ten)
    M.interpolate(as_matrix([[1e-6, 0, 0],[0, 1e-6, 0], [0, 0, 1e-2]]))
    check_spd(M)
...
def adapt_multiple(mesh, metric_fn, num, **kwargs):
    for i in range(num):
        metric = metric_fn(mesh, **kwargs)
        mesh = adapt(mesh, metric)
    return mesh


mp = {"dm_plex_metric": {"target_complexity": 100000.0, 
                        "p": 2, 
                        "a_max": 1000.0,
                        "h_min": 1.0,
                        "h_max": 10000.0,}}
newmesh = adapt_multiple(mesh, construct_metric, 1, metric_parameters=mp)
...
print(f"New Mesh vertex count: {mesh.num_vertices()}")
...
vp_fields = {'viscosity': mu, 'source': mom_source+horizontal_source, 'coriolis_frequency': f} #, 'interior_penalty': ip_alpha}
