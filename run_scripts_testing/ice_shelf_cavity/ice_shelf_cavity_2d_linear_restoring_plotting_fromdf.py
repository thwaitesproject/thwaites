...
firedrake_folder_layered = "/data/2d_mitgcm_comparison/01.07.20_3_eq_param_ufricHJ99_dt30.0_dtOutput3600.0_T432000.0_ip50.0_linearTres86400.0_Kh0.001_Kv0.0001_dy50_dz1_closed_iterative/"
...
output_folder = "/data/mitgcm_comparison_videos/linear_restoring/2d_5days_linear_restoring_diff_colour_scheme_01.07.20_3_eq_param_ufricHJ99_dt30.0_dtOutput3600.0_T432000.0_ip50.0_linearTres86400.0_Kh0.001_Kv0.0001_dy50_dz1_closed_iterative/"
##########
...
def MITgcm_Thwaites_comparison_plot(t, fields, same_colour_scale=False):
    fig = plt.figure(figsize=(20, 10))
    model = ["Original step-like restoring", "Linear restoring ramp"]
    ax = []
    ...
    if same_colour_scale:
        ...
        cplotMIT = ax[1].tricontourf(1e-3 * y_array_fd_layered, z_array_fd_layered, fields[1], cplotFD.levels, cmap='bwr',extend='both')
    else:
...
for t in range(115,116):#T_end_hours+1):
    ...
    temperature_plot = [temp_fd, temp_fd_layered, temp_levels, "Temperature / $^{\circ}$C", "temperature_"]
    salinity_plot = [sal_fd, sal_fd_layered, sal_levels, "Salinity / PSU", "salinity_"]
    v_plot = [v_fd, v_fd_layered, v_levels, "v (meridional) / m/s", "v_velocity_"]
    w_plot = [w_fd, w_fd_layered, w_levels, "w (vertical) / m/s", "w_velocity_"]
    mag_vel_plot = [vel_mag_fd, vel_mag_fd_layered, vel_mag_levels, "|u| / m/s", "mag_velocity_"]
