...
# create mesh
mesh = Mesh("./unstructured_cavity_1m_isotropic.msh")
x = mesh.coordinates.dat.data[:,0]
mesh.coordinates.dat.data[:,0] = 50 * x
...
source_temp = conditional(y > (1.0-sponge_fraction) * L,
                          absorption_factor * T_restore,#((y - (1.0-sponge_fraction) * L)/(L * sponge_fraction)) * absorption_factor * T_restore,
                          0.0)

# Salinity source term
source_sal = conditional(y > (1.0-sponge_fraction) * L,
                         absorption_factor * S_restore,#((y - (1.0-sponge_fraction) * L)/(L * sponge_fraction)) * absorption_factor * S_restore,
                         0.0)

# Temperature absorption term
absorp_temp = conditional(y > (1.0-sponge_fraction) * L,
                          absorption_factor,#((y - (1.0-sponge_fraction) * L)/(L * sponge_fraction)) * absorption_factor,
                          0.0)
...
folder = "/data/2d_mitgcm_comparison/"+str(args.date)+"_3_eq_param_ufricHJ99_dt"+str(dt)+\
         ...
         +"_dy50_dz1_unstructured_closed_iterative/"
