...
from thwaites.utility import get_top_boundary, cavity_thickness, CombinedSurfaceMeasure
...
parser.add_argument("T", help="final simulation time in seconds",
        type=float)
parser.add_argument("angle", help="angle of the crevasse wrt flow direction (along/across)")
...
crevasse_width = 250
crevasse_top_width = 50
crevasse_centre = 2500
crevasse_left = crevasse_centre - crevasse_width / 2
crevasse_right = crevasse_centre + crevasse_width / 2
crevasse_top_left = crevasse_centre - crevasse_top_width / 2
crevasse_top_right = crevasse_centre + crevasse_top_width / 2
water_depth = 600
ocean_thickness_base = 100
ice_draft_outside = -500
ice_draft_inside = -450
if not DUMP:
    with CheckpointFile(f"crevasse_{args.angle}_surface_mesh_5km_250to50to5m.h5", 'r') as chk:
        base_mesh = chk.load_mesh("crevasse_surface_mesh") 
    layers = 20
    dz = ocean_thickness_base / layers
    ...
    if args.angle == 'across':
        coord = x
    else:
        coord = y
    ice_draft.interpolate(conditional(coord < crevasse_left, ice_draft_outside,
                          conditional(coord < crevasse_top_left, ice_draft_outside+(ice_draft_inside-ice_draft_outside)*(coord-crevasse_left)/(crevasse_top_left-crevasse_left),
                          conditional(coord < crevasse_top_right, ice_draft_inside,
                          conditional(coord < crevasse_right, ice_draft_inside+(ice_draft_outside-ice_draft_inside)*(coord-crevasse_top_right)/(crevasse_right-crevasse_top_right),
                              ice_draft_outside)))))
    ...
    x, y, z = SpatialCoordinate(mesh)
    # Make the top z = 0 so roms squashing function works!
    f = Function(Vc).interpolate(as_vector([x, y, z -ocean_thickness_base]))
    ...
    b = Constant(0)
    depth_c = 10.0
    z_scaled = z /  ocean_thickness_base
    Cs = (1.-b) * sinh(a*z_scaled) / sinh(a) + b*(tanh(a*(z_scaled + 0.5))/(2*tanh(0.5*a)) - 0.5)
    f = Function(Vc).interpolate(as_vector([x, y, depth_c*z_scaled + (ocean_thickness_base - depth_c)*Cs])) #+ (1.0 - offset_backward_step_approx(x,k,x0))*z]))
    mesh.coordinates.assign(f)
    x, y, z = SpatialCoordinate(mesh)
    # Undo make the top z = 0 so keep high vertical res at top
    f = Function(Vc).interpolate(as_vector([x, y, z +ocean_thickness_base]))
    mesh.coordinates.assign(f)
    x, y, z = SpatialCoordinate(mesh)
    # Squash the mesh to make the crevasse
    f = Function(Vc).interpolate(as_vector([x, y, ocean_thickness*z/ocean_thickness_base - -bathymetry]))
    mesh.coordinates.assign(f)
...
mesh_r_file = File("mesh_refinement_beforesquash.pvd")
...
Q_mixed = Function(S, name="ocean heat flux")
Q_ice = Function(S, name="ice heat flux")
Q_latent = Function(S, name="latent heat")
Q_s = Function(S, name="ocean salt flux")
Tb = Function(S, name="boundary freezing temperature")
Sb = Function(S, name="boundary salinity")
full_pressure = Function(M.sub(1), name="full pressure")
p_b = Function(W, name="balance pressure")

frazil = Function(S, name="frazil") # should this really be P0dg to prevent negative frazil ice?
frazil_flux = Function(S, name="frazil ice flux") 
##########
...
frazil_eq = FrazilAdvectionDiffusionEquation(S,S, quad_degree=qdeg)
balance_pressure_eq = BalancePressureEquation(W,W, quad_degree=qdeg)
##########
...
# mesh refinement parameters from crevasse_surface_mesh.py using 
# animate mesh refinement library
fine_dx = 5
med_dx = 50
coarse_dx = 250
# note that for mesh generation this is set to 1/2 crevasse width
# but with this here even though horizontal perpedciular direction 
# has ~7m max on edge of crevasse slope cf at centre the scaling 
# diffusivity by 20x
mesh_width = 4*crevasse_width

P1_ten = TensorFunctionSpace(mesh, "CG", 1)
mesh_size = Function(P1_ten)

if args.angle == 'across':
    anisotropic = as_matrix([[fine_dx,0 , 0], [0, med_dx, 0 ], [0, 0, fine_dx]])
    coord = x
else:
    anisotropic = as_matrix([[med_dx,0 , 0], [0, fine_dx, 0 ], [0, 0, fine_dx]])
    coord = y

coarse = as_matrix([[coarse_dx,0 , 0], [0, coarse_dx, 0 ], [0, 0, fine_dx]])

crevasse_refinement = exp(-(coord-crevasse_centre)**2/(2*mesh_width**2))  

mesh_size.interpolate(crevasse_refinement* anisotropic + (1-crevasse_refinement) * coarse)

# adjust horizontal diffusivity/ viscosity to match grid size.
# vertical and horizontal perpedicular to ice base on crevasse walls 
# should have diffusivity/viscosity of 1e-3, scaling to 50x and 10x larger 
# outside crevasse and along the crevasse respectively to match
# anisotropic surface mesh
kappa = Function(P1_ten).interpolate(1e-3*mesh_size/fine_dx)
mu = Function(P1_ten).interpolate(1e-3*mesh_size/fine_dx)

kappa_check_file = File("anisotropic_kappa_check.pvd")
kappa_check_file.write(kappa)
...
mp = ThreeEqMeltRateParam(sal, temp, p, z, velocity=pow(dot(vdg, vdg), 0.5), HJ99Gamma=True, ice_heat_flux=False)
...
vp_bcs = {'top': {'un': no_normal_flow, 'drag': ice_drag}, 4: {}, 
        1: {}, 'bottom': {'un': no_normal_flow, 'drag': 2.5e-3},
        2: {}, 3: {}} # gmsh bcs
...
temp_bcs = {'top': {'flux': -mp.T_flux_bc}, 3: {'qadv': T_restore}, 4: {'qadv': T_restore}, 1: {'qadv': T_restore}, 2: {'qadv': T_restore}}
...
sal_bcs = {'top': {'flux': -mp.S_flux_bc}, 3:{'qadv': S_restore}, 4:{'qadv': S_restore}, 1:{'qadv': S_restore}, 2:{'qadv': S_restore}}
...
# Set up folder
folder = f"/data/ice_ocean/3d_crevasse/16.05.24_test_extruded_crevasse/" # /g/data/xd2/ws9229/ice_ocean/3d_crevasse/{args.date}_5kmflume_3_eq_param_ufricHJ99_dt{dt}_dtOutput{output_dt}_T{T}_3d_{args.angle}_openbox_bodyforce_posv_qice0_dz5m_dx20mto250m_scalehorvis_consTS_qadv_balp_nocutoff_refdz2out/"
     ...
     #      depth_profile_to_csv(depth_profile2km, velocity_depth_profile2km, "2km", time_str)
      #     depth_profile_to_csv(depth_profile4km, velocity_depth_profile4km, "4km", time_str)
       #    depth_profile_to_csv(depth_profile6km, velocity_depth_profile6km, "6km", time_str)
           ...
           PETSc.Sys.Print("integrated melt =", assemble(melt * ds('top')))
