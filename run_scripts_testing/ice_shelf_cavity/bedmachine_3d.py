...
water_depth = 2000  # bottom already at - 1000 so depth now -3000m
...
PETSc.Sys.Print("P1 dofs (no of nodes):", P1.dim())
...
kappa = as_tensor([[1e-2, 0, 0], [0, 1e-2, 0], [0, 0, 1e-2]])
...
vp_fields = {'viscosity': mu, 'source': mom_source, 'coriolis_frequency': f} #, 'interior_penalty': ip_alpha}
...
# Set up folder
folder = "/data/3d_bedmachine/"+str(args.date)+"_3_eq_param_ufricHJ99_dt"+str(dt)+\
         "_dtOutput"+str(output_dt)+"_T"+str(T)+"_3d_isotropicdx200m_open_qice0_3000mdepth_frazil_iterative_nodragside_coriolis_noforcingflow/"
