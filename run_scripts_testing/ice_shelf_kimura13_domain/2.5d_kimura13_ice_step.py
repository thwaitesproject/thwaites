...
# beneath ice shelf. 50km cavity, 50km open ocean
...
# momentum source: the buoyancy term Boussinesq approx. From kimura 13
T_ref = Constant(1.0)
S_ref = Constant(35)
...
# coriolis frequency f-plane assumption at 70deg S according to kimura 13. i think that would be Omega_z = 7e-5 s^-1. 
# Kimura 13 give Omegaz = 5e-5 s^-1. f = 2*Omega_z
f = Constant(-1e-4)  
...
mp = ThreeEqMeltRateParam(sal, temp, p, z, velocity=pow(dot(v, v) + pow(u, 2), 0.5))
##########
...
u_bcs = {2: {'drag': ice_drag}, 3: {'drag': ice_drag}, 4: {'q': Constant(0.0)}, 
        1: {'drag': 0.0025}, 5: {'drag': ice_drag},
        6: {'drag': ice_drag}}
...
# Set up folder
folder = "/data/2.5d_kimura13_icestep/"+str(args.date)+"_3_eq_param_ufricJ10_dt"+str(dt)+\
         "_dtOutput"+str(output_dt)+"_T"+str(T)+"_ip"+str(ip_factor.values()[0])+\
         "_Kh"+str(kappa_h.values()[0])+"_Kv"+str(kappa_v.values()[0])\
         +"_2.5d_kimura13_closed_iterative/"
