...
#kappa_temp = conditional(x>L*0.96,2.0, 0.001+1.875E-5*x)
#kappa_sal = conditional(x>L*0.96,2.0, 0.001+1.875E-5*x)
#mu = conditional(x>L*0.96,2.0, 0.001+1.875E-5*x)



# Initial viscosity fields. Update based on function below every 10 (...) time steps.
#mu_min=2.0E-1 #
#mu_max = 2.0

#mu = 0.5*(mu_max+mu_min) + 0.5*(mu_max-mu_min)*tanh((x-4250.0)/150.0)
#kappa_temp = 0.5*(mu_max+mu_min) + 0.5*(mu_max-mu_min)*tanh((x-4250.0)/150.0)
#kappa_sal = 0.5*(mu_max+mu_min) + 0.5*(mu_max-mu_min)*tanh((x-4250.0)/150.0)

rho0=1025.0  # Holland and jenkins 1997) used later to recalculate the full pressure for meltrate with hydrostatic term

min_molecular_kin_visc = 1.95E-6 # from holland and Jenkins 1999 #1.8E-3/rho0
min_molecular_kappa_temp = 1.41E-7
min_molecular_kappa_sal = 8.02E-10

def grid_peclet(u,x):

    gradient_dx = 4.9E-3 # (5-0.1)/5000 already defined from mesh
    Pe_dx = 2.0
    return u*(0.1+gradient_dx*x)/Pe_dx
...
folder = "/data/thwaites/18.12.18.iSWING_2_eq_param_dt_864_t120_delT_0.1_change_p_split/"
...
mu_vis_h = Function(K)
mu_vis_h.interpolate(mu)


mu_vis_v = Function(K)
mu_vis_v.interpolate(mu)
#From Ben's thesis page 31
...
#q_ice, q_mixed, q_mixed_real, q_latent, q_s, wb, t_b, pfull = MeltRateParam(sal, temp, p,cavity_depth,z,dz).three_eq_param_meltrate_friction_vel(u)

# two eq param to test if still freezing...
q_ice, q_mixed, q_mixed_real, q_latent, q_s, wb, t_b, pfull = MeltRateParam(sal, temp, p,cavity_depth,z,dz).two_eq_param_meltrate()
...
mu_vis_h_file = File(folder+"mu_vis_hor.pvd")
mu_vis_h_file.write(mu_vis_h)
mu_vis_v_file = File(folder+"mu_vis_vert.pvd")
mu_vis_v_file.write(mu_vis_v)
...
while t < T - 0.5*dt:
    ...
    if step % 5 == 0:
        # update viscosity and diffusivities
        grid_mu_hor = grid_peclet(abs(u[0]),x)
        mu_hor = conditional(grid_mu_hor > min_molecular_kin_visc,grid_mu_hor,min_molecular_kin_visc)
        kappa_temp_hor = conditional(grid_mu_hor > min_molecular_kappa_temp,grid_mu_hor,min_molecular_kappa_temp)
        kappa_sal_hor = conditional(grid_mu_hor> min_molecular_kappa_sal,grid_mu_hor,min_molecular_kappa_sal)

        grid_mu_vert = grid_peclet(abs(u[1]), x)
        mu_vert = conditional(grid_mu_vert > min_molecular_kin_visc, grid_mu_vert, min_molecular_kin_visc)
        kappa_temp_vert = conditional(grid_mu_vert > min_molecular_kappa_temp, grid_mu_vert, min_molecular_kappa_temp)
        kappa_sal_vert = conditional(grid_mu_vert > min_molecular_kappa_sal, grid_mu_vert, min_molecular_kappa_sal)

        mu_vis_h.interpolate(mu_hor)
        mu_vis_v.interpolate(mu_hor)

        mu = as_matrix([[mu_hor, 0.0],
                        [0.0, mu_hor]])

        kappa_temp = as_matrix([[kappa_temp_hor, 0.0],
                                [0.0, kappa_temp_hor]])

        kappa_sal = as_matrix([[kappa_sal_hor, 0.0],
                               [0.0, kappa_sal_hor]])
