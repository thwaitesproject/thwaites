...
#mesh.coordinates.dat.data[:, 1] += -h_cav


z = z -h_cav
#z_vector = interpolate(x, Function(DG_2d)).dat.data, interpolate(z, Function(DG_2d)).dat.data

z_vector = interpolate(z,Function(Q))#.dat.data
#x_vector = interpolate(x,Function(Q)).dat.data
...
#for i in range(len(z_vector)):
 #   if x_vector[i] ==5000:
 #       z_rhs.append(float(z_vector[i]))
        
#print(z_rhs)


#print(sorted(z_rhs))
#print(z_vector[0], z_vector[-1])
...
kappa_temp = Constant(10)
kappa_sal = Constant(10)
#mu = Constant(1e-3)
mu = Constant(10)  # kinematic



temp_init = Constant(-2.0)   # Tinit =Tres deg C from ben's thesis page 54
temp.interpolate(temp_init)


sal_init = Constant(34.5) # stationay flow S = Srest#set this to slightly less than Sres. horizontal pressure gradient increases as depth increases. pressure greater outside domain so drives flow in!
...
folder = "./20.11.18.top_heat_flux_ice_overburden3/"
...
z_file = File(folder+"z_values.pvd")
z_file.write(z_vector)
...
def meltrate(S, T, Pin):
    ...
    return Q_ice,Q_mixed_bc,Q_latent,wb,Tb # these are all still expressions



temp_bcs = {2: {'q': Trestore}, 3: {'flux': Q_mixed}}
temp_solver_parameters = mumps_solver_parameters


wb =Constant(0)
#sal_right = 40.0
sal_bcs ={2: {'q': Srestore}, 3: {'flux': wb*sal}} # {1: {'q': sal_ice}}
...
while t < T - 0.5*dt:
    ...
    q_ice, q_mixed, q_latent, wb,t_b = meltrate(sal,temp,p_)
