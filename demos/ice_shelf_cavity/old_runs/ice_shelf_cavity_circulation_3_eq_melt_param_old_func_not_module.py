...
folder = "/data/thwaites/26.11.18.ice_shelf_cavity_circulation_3_eq_param_module1/"
...
def three_eq_param_meltrate(S, T, Pin):
    ...
    Q_mixed_bc = conditional(z > 0 - dz_calc, -(wb + gammaT) * (loc_Tb - T),
                             0.0)  # units of K m/s , add in meltrate to capture flux of water through boundary Jenkins et al 2001 eq 25
    Q_s_bc = conditional(z > 0 - dz_calc, -(wb + gammaS) * (loc_Sb - S), 0.0)
...
#if mesh.comm.rank == 0:
#    Q_mixed_vector = Q_mixed([shelf_boundary_points])
