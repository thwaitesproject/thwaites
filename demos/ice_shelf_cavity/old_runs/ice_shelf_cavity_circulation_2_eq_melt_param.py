...
mesh = Mesh("/data/thwaites/meshes/mesh/mesh.msh")#ice_shelf_mesh5.msh") #SquareMesh(nx, nz, L) #Mesh("ice_shelf_mesh3.msh")#
...
folder = "/data/thwaites/23.11.18.ice_shelf_cavity_circulation1/"
...
temp_test = TestFunction(Q)
#temp_eq = ScalarAdvectionDiffusionEquation(temp_test, K)
temp_eq = ScalarAdvectionDiffusionEquation(Q, Q)


sal_test = TestFunction(Q)
...
interp_source_temp = Function(Q)
interp_source_sal = Function(Q)
...
#absorption term
interp_absorp_temp = Function(Q)
interp_absorp_sal = Function(Q)
...
q_ice, q_mixed, q_mixed_real, q_latent, q_s, wb, t_b, pfull = meltrate(sal, temp, p_)  # these are expressions
